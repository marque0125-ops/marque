              ...
              <div className="flex justify-between items-center gap-4">
                ...
                <div className="flex gap-2">
                  {order.status !== 'delivered' && order.status !== 'cancelled' && (
                    <button onClick={() => advanceOrderStatus(order.id)} className="bg-brand-orange text-black px-3.5 py-1.5 rounded font-bold uppercase text-[9px] hover:bg-brand-gold">
                      Fulfill / Advance Status
                    </button>
                  )}
                  {order.status !== 'cancelled' && order.status !== 'delivered' && (
                    <button onClick={() => cancelOrder(order.id)} className="border border-brand-border hover:border-red-500 text-slate-400 hover:text-red-500 px-3.5 py-1.5 rounded font-bold uppercase text-[9px]">
                      Cancel & Refund
                    </button>
                  )}
                </div>
