...
export function InventoryTab() {
  const { products, addProduct, updateProduct, deleteProduct, updateProductStock } = useProductStore();
  ...
  const [formImageUrl, setFormImageUrl] = useState("https://images.unsplash.com/photo-1594787318286-3d835c1d207f?w=600&q=80");
  ...
  const handleOpenEditProduct = (p: any) => {
    ...
    setFormWeightGrams(p.weightGrams || 3000); setFormDescription(p.description); setFormImageUrl(p.images[0] || "");
  ...
  const handleSaveProduct = (e: React.FormEvent) => {
    ...
    const productData: any = {
      ...
      images: [formImageUrl], whatsInTheBox: formWhatsInTheBox.split(",").map(i => i.trim()),
  ...
  const handleAddVariant = () => {
    ...
    const newV = {
      ...
      stockQty: newVarStock, attributes: { color: newVarColor, battery: newVarBattery }
    };
        ...
        <form onSubmit={handleSaveProduct} className="space-y-6 bg-slate-900/60 p-6 rounded-2xl border border-brand-border backdrop-blur-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs text-slate-300">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <option value="street-basher">Street Basher</option>
                <option value="speed-car">Speed Car</option>
                <option value="off-road-truggy">Off-Road Truggy</option>
                <option value="scale-crawler">Scale Crawler</option>
            ...
            <div className="space-y-1.5 md:col-span-2"><label className="text-[10px] text-slate-400 font-bold uppercase block">Image URL</label><input type="text" value={formImageUrl} onChange={(e) => setFormImageUrl(e.target.value)} required className="w-full rounded-lg bg-slate-950 border border-brand-border py-2 px-3 focus:border-brand-orange" /></div>
          </div>
          <div className="border-t border-brand-border pt-4 text-xs space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <input type="text" value={newVarBattery} onChange={e => setNewVarBattery(e.target.value)} placeholder="Battery" className="w-32 rounded bg-slate-900 border border-brand-border p-1.5 text-white" />
