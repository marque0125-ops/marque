    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="rounded-2xl border border-brand-border bg-slate-950 p-6 space-y-2">
          <span className="text-[10px] text-slate-400 font-normal uppercase tracking-wider">Gross Merchandise Value (GMV)</span>
          ...
          <p className="text-[10px] text-slate-400">Excludes cancelled order volumes</p>
        </div>

        <div className="rounded-2xl border border-brand-border bg-slate-950 p-6 space-y-2">
          <span className="text-[10px] text-slate-400 font-normal uppercase tracking-wider">Average Order Value (AOV)</span>
          <div className="flex items-baseline gap-1.5 leading-none">
            <span className="text-3xl font-normal text-brand-gold font-display">₹{aov.toLocaleString('en-IN')}</span>
          </div>
          <p className="text-[10px] text-slate-400">Based on {totalOrders} placed manifests</p>
        </div>

        <div className="rounded-2xl border border-brand-border bg-slate-950 p-6 space-y-2">
          <span className="text-[10px] text-slate-400 font-normal uppercase tracking-wider">Store Conversion Rate</span>
          <div className="flex items-baseline gap-1.5 leading-none">
          ...
          <p className="text-[10px] text-slate-400">Typesense Instant Search speed &lt; 80ms</p>
        </div>

        <div className="rounded-2xl border border-brand-border bg-slate-950 p-6 space-y-2">
          <span className="text-[10px] text-slate-400 font-normal uppercase tracking-wider">Manifest Orders Placed</span>
          <div className="flex items-baseline gap-1.5 leading-none">
            <span className="text-3xl font-normal text-brand-orange font-display">{totalOrders}</span>
          </div>
          <p className="text-[10px] text-slate-400">Fulfillment managed via Admin Command Console</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        <div className="md:col-span-8 rounded-2xl border border-brand-border bg-slate-950 p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-brand-border pb-3">
            <h2 className="font-display text-sm font-normal uppercase tracking-wider text-slate-200 flex items-center gap-1.5">
              <AlertTriangle className="h-4.5 w-4.5 text-brand-orange" />
              Telemetry Low Stock alerts board ({lowStockAlerts.length})
            </h2>
            {lowStockAlerts.length > 0 && (
              <button onClick={clearLowStockAlerts} className="text-[10px] text-slate-400 hover:text-brand-orange font-normal uppercase underline">
                Reset log
              </button>
            )}
          </div>
            ...
            {lowStockAlerts.length === 0 ? (
              ...
              lowStockAlerts.map((alert, idx) => (
                <div key={idx} className="p-3 rounded bg-slate-900 border-l border-brand-orange text-xs text-slate-300 flex justify-between gap-4">
                  ...
                  <span className="font-mono text-[9px] text-slate-400 shrink-0 font-normal">{alert.date}</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className="md:col-span-4 rounded-2xl border border-brand-border bg-slate-950 p-6 space-y-6">
          <div className="space-y-4">
            <h2 className="font-display text-sm font-normal uppercase tracking-wider text-slate-200 border-b border-brand-border pb-3 flex items-center gap-1.5">
