  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-950 p-6 rounded-2xl border border-brand-border">
        ...
        {!isAdding && !isEditing && (
          <button onClick={() => { resetForm(); setIsAdding(true); }} className="bg-brand-orange text-black px-4 py-2.5 rounded-lg text-xs font-bold uppercase flex items-center gap-1.5 hover:bg-brand-gold transition-colors">
        ...
        <form onSubmit={handleSave} className="space-y-6 bg-slate-900/60 p-6 rounded-2xl border border-brand-border backdrop-blur-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-slate-300">
            <div className="space-y-1.5">
              <label className="text-[10px] text-slate-400 font-bold uppercase block">Category ID (System Key)</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] text-slate-400 font-bold uppercase block">Display Name</label>
            ...
            <div className="space-y-1.5 md:col-span-2">
              <label className="text-[10px] text-slate-400 font-bold uppercase block">Category Image</label>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-brand-border">
            <button type="button" onClick={resetForm} className="border border-brand-border px-4 py-2 rounded-lg text-xs font-bold uppercase text-slate-400 hover:bg-slate-800 transition-colors">Cancel</button>
            <button type="submit" className="bg-brand-orange px-5 py-2 rounded-lg text-xs font-bold uppercase text-black hover:bg-brand-gold transition-colors">{isEditing ? "Save Changes" : "Create Category"}</button>
          </div>
        </form>
      ) : (
        <div className="rounded-2xl border border-brand-border bg-slate-950 overflow-hidden">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-900 border-b border-brand-border text-[10px] uppercase font-bold text-slate-400 tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-brand-border/40 text-slate-200">
              {categories.map(cat => {
                ...
                return (
                  <tr key={cat.id} className="hover:bg-slate-900/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-bold text-sm text-white">{cat.name}</td>
                    <td className="px-6 py-4 font-mono text-brand-orange">{cat.id}</td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2 py-1 rounded bg-slate-800 border border-brand-border text-[10px] font-bold">
                        {linkedCount} items
                      </span>
                    </td>
              ...
              {categories.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500 font-bold uppercase tracking-wider">
                    No Categories Found
                  </td>
                </tr>
              )}
