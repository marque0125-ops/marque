      ...
      <div className="relative overflow-hidden rounded-3xl border border-brand-border bg-gradient-to-br from-slate-950 via-slate-900/60 to-slate-950 p-8 sm:p-12 shadow-2xl">
        ...
        <div className="relative max-w-2xl space-y-4">
          <div className="flex items-center gap-2 text-brand-orange">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest font-display">Premium Collection Showcase</span>
          </div>
          ...
          {/* Quick trust metrics */}
          <div className="flex flex-wrap gap-4 pt-2 text-[10px] text-slate-400 font-bold uppercase tracking-wider">
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <aside className="w-full lg:w-64 shrink-0 rounded-2xl border border-brand-border bg-slate-950 p-6 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Brand Lineup</label>
            <div className="flex flex-col gap-1.5">
              {[
                ...
                <button
                  ...
                  className={`w-full py-2.5 px-3 text-left text-xs font-bold uppercase rounded-lg border transition-all ${activeBrandTab === btn.id ? 'bg-brand-orange text-black border-brand-orange' : 'border-brand-border bg-slate-900/50 hover:bg-slate-900 text-slate-300'}`}
                >
                  {btn.label}
                </button>
              ))}
          ...
          <label className="flex items-center gap-2.5 cursor-pointer text-xs text-slate-300 font-medium select-none bg-slate-900/40 p-3 rounded-lg border border-brand-border/40 hover:border-brand-border transition-colors">
            <input 
              type="checkbox" 
              checked={onlyInStock}
        ...
        <div className="flex-grow space-y-6 w-full">
          {filteredAccessories.length === 0 ? (
            <div className="rounded-2xl border border-brand-border bg-slate-950 py-24 text-center space-y-4">
              ...
              <h3 className="font-display text-lg font-bold text-white uppercase">No items match filters</h3>
                  ...
                  >
                    ...
                    <div className="relative aspect-square w-full overflow-hidden bg-slate-950">
                      ...
                      <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
                        <span className="rounded bg-black/85 px-2.5 py-0.5 text-[9px] font-black text-brand-orange border border-brand-orange/30 uppercase tracking-widest font-display">
                          {brand?.name}
                        </span>
                        {p.specs.Pieces && (
                          <span className="rounded bg-black/85 px-2 py-0.5 text-[9px] font-bold text-brand-gold border border-brand-gold/30 uppercase tracking-wider">
                            {p.specs.Pieces}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="absolute bottom-3 left-3 z-10">
                        {p.stockQty === 0 ? (
                          <span className="rounded bg-red-600 px-2 py-0.5 text-[9px] font-black text-white uppercase tracking-wider">
                            SOLD OUT
                          </span>
                        ) : p.stockQty < 3 ? (
                          <span className="rounded bg-brand-orange px-2 py-0.5 text-[9px] font-black text-black uppercase tracking-wider animate-pulse">
                            Only {p.stockQty} Left!
                          </span>
                        ) : (
                          <span className="rounded bg-slate-950/85 border border-slate-800 px-2 py-0.5 text-[9px] font-bold text-green-400 uppercase tracking-wider">
                            Ready To Ship
                          </span>
                        )}
                      </div>
                    ...
                    <div className="p-4 flex-grow flex flex-col justify-between gap-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px] bg-slate-950/50 p-2 rounded-lg border border-brand-border/40 font-semibold text-slate-300 leading-none">
                      ...
                      <div className="flex items-center justify-between border-t border-brand-border/60 pt-3">
                        <div>
                          ...
                          <span className="text-base font-black text-brand-gold font-display leading-none block mt-1.5">
                        ...
                        <button 
                          ...
                          className="rounded-lg bg-brand-orange text-black px-3.5 py-2.5 text-xs font-bold uppercase hover:bg-brand-gold disabled:bg-slate-800 disabled:text-slate-600 transition-colors flex items-center gap-1.5"
                        >
